...
const runFCFS = () => {
  ...
  const runSimulation = async () => {
    ...
    try {
      const res = await axios.post('http://localhost:5000/api/fcfs', {
        processes: filled,
      });
...
const simulateFCFS = (procList) => {
  const processes = [...procList].map(p => ({
    ...p,
    arrivalTime: Number(p.arrivalTime),
    burstTime: Number(p.burstTime),
  }));
  ...
  setResults(results); // assuming setResults is your state setter
  setShowChart(true);  // to trigger chart rendering
};
