...
export const worstFitAlgorithm = (
  ...
  while (waitingProcesses.length > 0 || completedProcesses.some(p => p.allocated)) {
    ...
    for (const process of arrivedProcesses) {
      // Worst-fit: Find the largest block that's big enough
      const freeBlocks = currentMemoryState.filter(block => !block.allocated && block.size >= process.size);
      
      if (freeBlocks.length > 0) {
        // Sort blocks by size in descending order to find the worst fit (largest block)
        const worstFitBlock = freeBlocks.sort((a, b) => b.size - a.size)[0];
        const blockIndex = currentMemoryState.findIndex(block => block.id === worstFitBlock.id);
        ...
        if (blockIndex !== -1) {
          ...
          steps.push({
            ...
            currentAction: `Allocated memory for Process ${process.id} at address ${block.startAddress} (Worst Fit)`
          });
        }
      }
    }
