...
export const firstFitAlgorithm = (
  ...
  while (waitingProcesses.length > 0 || completedProcesses.some(p => p.allocated)) {
    ...
    for (const process of arrivedProcesses) {
      // First-fit: Allocate the first block that's big enough
      for (let i = 0; i < currentMemoryState.length; i++) {
        const block = currentMemoryState[i];
        ...
        if (!block.allocated && block.size >= process.size) {
          ...
          steps.push({
            ...
            currentAction: `Allocated memory for Process ${process.id} at address ${block.startAddress}`
          });
